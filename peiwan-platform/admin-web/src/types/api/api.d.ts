...
declare namespace Api {
  ...
  namespace Business {
    ...
    interface ServiceItem { id:number;gameId:number;gameName:string;serviceCode:string;serviceName:string;serviceType:string;description?:string;sortNo:number;enabled:boolean;createTime?:string;levelPrices?:Array<{id?:number;playerLevelId:number;unitType:'HOUR'|'GAME'|'ORDER';price:number;marketPrice?:number;enabled:boolean}> }
    interface ProductSku { id?:number;skuCode:string;skuName:string;price:number;marketPrice?:number;unitType:'HOUR'|'GAME'|'ORDER';unitCount:number;playerCount:number;priceType:'PER_PLAYER'|'FIXED_TOTAL';minQuantity:number;maxQuantity?:number;stockMode:'UNLIMITED'|'LIMITED';stockQuantity?:number;serviceMinutes?:number;enabled:boolean;sortNo:number }
    ...
    interface Order { id:number;orderNo:string;customerId:number;customerUsername:string;customerNickname?:string;assignedPlayerId?:number;playerName?:string;productName?:string;orderStatus:string;requiredPlayerCount?:number;memberCount?:number;totalAmount:number;payableAmount:number;paidAmount:number;contactName?:string;contactPhone?:string;customerRemark?:string;cancelReason?:string;createTime:string;items?:OrderItem[];members?:OrderMember[];gameProfile?:OrderGameProfile;statusLogs?:OrderStatusLog[] }
