  ...
  <div class="art-full-height">
    ...
    <ElCard class="art-table-card">
      <ArtTableHeader :loading="loading" @refresh="load">
        <template #left><ElButton v-auth="'business:player:create'" @click="open()">新增陪玩师</ElButton><ElButton v-auth="'business:player:audit'" type="warning" plain @click="profileReview=true">资料变更审核</ElButton><ElButton type="primary" plain @click="openApplications">入驻申请</ElButton></template>
      </ArtTableHeader>
    ...
    <ElDialog v-model="applicationVisible" title="陪玩师入驻申请" width="1050px" destroy-on-close>
      <ElAlert class="mb-4" type="info" :closable="false" title="这里只记录用户联系方式并转人工跟进；处理申请不会自动创建陪玩师或授予陪玩师身份。" />
      <ElForm inline><ElFormItem label="关键词"><ElInput v-model="applicationQuery.keyword" clearable placeholder="姓名/电话/地址" /></ElFormItem><ElFormItem label="状态"><ElSelect v-model="applicationQuery.status" clearable class="!w-32"><ElOption label="待联系" value="PENDING" /><ElOption label="跟进中" value="PROCESSING" /><ElOption label="已关闭" value="CLOSED" /></ElSelect></ElFormItem><ElFormItem><ElButton type="primary" @click="loadApplications">查询</ElButton></ElFormItem></ElForm>
      <ElTable v-loading="applicationLoading" :data="applicationRows" max-height="520"><ElTableColumn prop="realName" label="姓名" width="110" /><ElTableColumn prop="phone" label="电话" width="135" /><ElTableColumn prop="address" label="地址" min-width="230" show-overflow-tooltip /><ElTableColumn label="状态" width="100"><template #default="{row}"><ElTag :type="applicationStatusType(row.status)">{{applicationStatusText(row.status)}}</ElTag></template></ElTableColumn><ElTableColumn prop="followUpRemark" label="跟进备注" min-width="160" show-overflow-tooltip /><ElTableColumn prop="createdAt" label="申请时间" width="175" /><ElTableColumn label="操作" width="145" fixed="right"><template #default="{row}"><ElButton v-if="row.status==='PENDING'" link type="primary" @click="handleApplication(row,'PROCESSING')">开始跟进</ElButton><ElButton v-if="row.status!=='CLOSED'" link type="danger" @click="handleApplication(row,'CLOSED')">关闭</ElButton></template></ElTableColumn></ElTable>
      <div class="flex justify-end mt-4"><ElPagination v-model:current-page="applicationQuery.current" v-model:page-size="applicationQuery.size" :total="applicationTotal" layout="total, prev, pager, next" @change="loadApplications" /></div>
    </ElDialog>
  </div>
</template>
...
import { auditPlayer, fetchPlayerApplications, fetchPlayers, handlePlayerApplication, setPlayerStatus } from '@/api/business-manage'
...
const applicationVisible=ref(false),applicationLoading=ref(false),applicationRows=ref<any[]>([]),applicationTotal=ref(0)
...
async function openApplications(){applicationVisible.value=true;applicationQuery.current=1;await loadApplications()}
async function loadApplications(){applicationLoading.value=true;try{const data=await fetchPlayerApplications(applicationQuery);applicationRows.value=data.records;applicationTotal.value=data.total}finally{applicationLoading.value=false}}
async function handleApplication(row:any,status:'PROCESSING'|'CLOSED'){const title=status==='PROCESSING'?'开始人工跟进':'关闭申请';const result=await ElMessageBox.prompt('请输入跟进备注',title,{inputPlaceholder:status==='PROCESSING'?'如：已电话联系，等待补充资料':'如：用户暂不考虑或无法联系',inputValidator:value=>!!value?.trim()||'请填写跟进备注'});await handlePlayerApplication(row.id,{status,remark:result.value});ElMessage.success('处理成功');await loadApplications()}
const applicationStatusText=(status:string)=>({PENDING:'待联系',PROCESSING:'跟进中',CLOSED:'已关闭'}[status]||status)
const applicationStatusType=(status:string)=>(status==='PROCESSING'?'warning':status==='CLOSED'?'info':'danger') as any
