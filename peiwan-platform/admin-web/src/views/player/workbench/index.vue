<template>
  <div class="art-full-height" v-loading="loading">
    ...
    <template v-else-if="dashboard">
      ...
      <ElCard shadow="never"><ElTabs v-model="tab"><ElTabPane label="待响应派单" name="dispatch"><ElTable :data="dispatches" empty-text="暂无待响应派单"><ElTableColumn prop="taskNo" label="任务号" min-width="180"/><ElTableColumn prop="gameName" label="游戏" width="120"/><ElTableColumn prop="productName" label="商品" min-width="150"/><ElTableColumn prop="skuName" label="规格" min-width="120"/><ElTableColumn label="接单进度" width="105"><template #default="{row}">{{row.memberCount || 0}}/{{row.requiredPlayerCount || 1}} 人</template></ElTableColumn><ElTableColumn prop="serverName" label="区服" width="110"/><ElTableColumn prop="rankName" label="段位" width="100"/><ElTableColumn label="响应截止" width="180"><template #default="{row}">{{formatDateTime(row.deadlineAt)}}</template></ElTableColumn><ElTableColumn label="操作" width="150" fixed="right"><template #default="{row}"><ElButton v-auth="'player:dispatch:accept'" link type="primary" @click="accept(row)">接受</ElButton><ElButton v-auth="'player:dispatch:reject'" link type="danger" @click="reject(row)">拒绝</ElButton></template></ElTableColumn></ElTable></ElTabPane><ElTabPane label="我的订单" name="orders"><ElTable :data="orders"><ElTableColumn prop="orderNo" label="订单号" min-width="190"/><ElTableColumn prop="gameName" label="游戏" width="120"/><ElTableColumn prop="productName" label="商品" min-width="160"/><ElTableColumn prop="skuName" label="规格" min-width="130"/><ElTableColumn label="成员" width="90"><template #default="{row}">{{row.memberCount || 0}}/{{row.requiredPlayerCount || 1}} 人</template></ElTableColumn><ElTableColumn label="状态" width="110"><template #default="{row}"><ElTag>{{orderText(row)}}</ElTag></template></ElTableColumn><ElTableColumn label="接单时间" width="180"><template #default="{row}">{{formatDateTime(row.assignedAt)}}</template></ElTableColumn><ElTableColumn label="操作" width="150"><template #default="{row}"><ElButton v-if="canStart(row)" link type="primary" @click="start(row)">开始服务</ElButton><ElButton v-if="canSubmit(row)" link type="success" @click="submit(row)">提交完成</ElButton></template></ElTableColumn></ElTable><div class="flex justify-end mt-4"><ElPagination v-model:current-page="query.current" v-model:page-size="query.size" :total="total" @change="loadOrders" layout="total, prev, pager, next"/></div></ElTabPane></ElTabs></ElCard>
    </template>
...
const canStart=(r:any)=>['ASSIGNED','IN_SERVICE'].includes(r.orderStatus)&&r.memberStatus==='ACCEPTED',canSubmit=(r:any)=>r.orderStatus==='IN_SERVICE'&&r.memberStatus==='IN_SERVICE'
const workText=(s:string)=>({AVAILABLE:'可接单',OFFLINE:'休息',BUSY:'已达接单上限',SUSPENDED:'暂停'}[s]||s),orderText=(r:any)=>canStart(r)&&r.orderStatus==='IN_SERVICE'?'待本人开始':(({ASSIGNED:'已接单',IN_SERVICE:'服务中',PENDING_CONFIRM:'待确认完成',COMPLETED:'已完成',CANCELLED:'已取消'}as Record<string,string>)[String(r.orderStatus)]||r.orderStatus)
onMounted(load)
</script>
