...
const activeOrders=computed(()=>orders.value.filter(item=>['ASSIGNED','IN_SERVICE','PENDING_CONFIRM','WAIT_CUSTOMER_CONFIRM','AFTER_SALE'].includes(String(item.orderStatus))))
...
function statusText(value:unknown){return({ASSIGNED:'服务待开始',IN_SERVICE:'服务进行中',PENDING_CONFIRM:'完成凭证待审核',WAIT_CUSTOMER_CONFIRM:'等待顾客确认',AFTER_SALE:'订单售后处理中'}as Record<string,string>)[String(value)]||'服务单状态更新'}
</script>
<style scoped>
