...
export const createPlayerServiceException = (id: number, data: { requestType: 'TRANSFER' | 'ABORT'; targetPlayerId?: number; reason: string; proofUrls?: string[] }) => request<{ id: number }>({ url: `/api/player/orders/${id}/service-exceptions`, method: 'POST', data })
export const getPlayerTransferTargets = (id: number) => request<RecordData[]>({ url: `/api/service-orders/${id}/transfer-targets`, method: 'GET' })
export const getPendingServiceTransfers = () => request<RecordData[]>({ url: '/api/player/service-transfers/pending', method: 'GET' })
export const respondServiceTransfer = (id: number, action: 'ACCEPT' | 'REJECT') => request<void>({ url: `/api/player/service-transfers/${id}/respond`, method: 'PUT', data: { action } })
