<template>
  <view class="settlement-page">
    ...
    <view v-for="item in earnings" :key="String(item.id)" class="earning-card">
      <view><text>{{ item.earningNo || '订单收入' }}</text><label>{{ formatTime(item.settledAt || item.createdAt) }}</label></view>
      <view><text>+¥{{ money(item.playerAmount) }}</text><label>订单 ¥{{ money(item.orderAmount) }}</label></view>
    </view>
