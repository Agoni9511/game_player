<template>
  <view class="select-page">
    ...
    <view v-for="player in players" :key="String(player.id)" class="player-card" :class="{ disabled: player.workStatus !== 'AVAILABLE', selected: selectedIds.includes(Number(player.id)) }" @click="selectPlayer(player)">
      ...
      <view class="arrow">{{ selectedIds.includes(Number(player.id)) ? '✓' : '›' }}</view>
    </view>

    <view v-if="loading" class="empty">正在寻找合适的陪玩师...</view>
    <view v-else-if="!players.length" class="empty">当前条件下暂无陪玩师，可以放宽筛选或发布到大厅</view>
    <view v-if="selectedIds.length" class="selection-bar"><text>已选 {{ selectedIds.length }}/{{ playerCount }} 人</text><button @click="confirmSelected">确认指定</button></view>
  </view>
</template>
...
function selectPlayer(player: RecordData) {
  ...
  if (playerCount.value <= 1) return goConfirm(Number(player.playerLevelId), [Number(player.id)], [String(player.nickname || '所选陪玩师')])
  const id = Number(player.id); const level = Number(player.playerLevelId || 0)
  if (selectedIds.value.length && !selectedIds.value.includes(id)) { const first = players.value.find(x => Number(x.id) === selectedIds.value[0]); if (Number(first?.playerLevelId || 0) !== level) return uni.showToast({ title: '请选择同一陪玩等级的陪玩师', icon: 'none' }) }
  if (selectedIds.value.includes(id)) selectedIds.value = selectedIds.value.filter(x => x !== id)
  else if (selectedIds.value.length >= playerCount.value) uni.showToast({ title: `最多选择 ${playerCount.value} 人`, icon: 'none' })
  else selectedIds.value = [...selectedIds.value, id]
}
function confirmSelected() { const rows = selectedIds.value.map(id => players.value.find(x => Number(x.id) === id)).filter(Boolean) as RecordData[]; if (!rows.length) return; goConfirm(Number(rows[0].playerLevelId), rows.map(x => Number(x.id)), rows.map(x => String(x.nickname || '所选陪玩师'))) }
function goConfirm(playerLevelId: number, requestedPlayerIds: number[] = [], requestedPlayerNames: string[] = []) {
  const ids = requestedPlayerIds.join(','); const names = requestedPlayerNames.length ? `&requestedPlayerNames=${encodeURIComponent(requestedPlayerNames.join(','))}` : ''
  uni.navigateTo({ url: `/subpackages/customer/confirm-order?productId=${product.value.id}&skuId=${selectedSku.value.id}&playerLevelId=${playerLevelId}&requestedPlayerIds=${ids}${names}` })
}
