  ...
  <view class="detail-page">
    ...
    <view class="content">
      <view class="card intro-card">
        ...
        <view class="price"><text class="currency">¥</text><text class="price-value">{{ money(selectedSku.price || product.minPrice) }}</text><text class="price-unit">/ {{ skuUnitLabel(selectedSku.unitType, selectedSku.unitCount) }}</text></view>
      </view>
      ...
      <view class="card">
        ...
        <view class="sku-list">
          <view v-for="sku in skus" :key="String(sku.id)" class="sku" :class="{ active: Number(selectedSku.id) === Number(sku.id) }" @click="selectedSku = sku">
            <view><text>{{ sku.skuName }}</text><label v-if="sku.serviceMinutes">约 {{ sku.serviceMinutes }} 分钟</label></view><text class="sku-price">¥{{ money(sku.price) }}</text>
          </view>
        </view>
      </view>
    ...
    <view class="action-bar"><view class="service" @click="contact"><image src="/static/icons/headset.png"/><text>客服</text></view><view class="buy" @click="nextStep">立即下单</view></view>
  </view>
</template>
