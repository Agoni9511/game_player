      ...
      <view class="card">
        ...
        <view class="sku-list">
          <view v-for="sku in skus" :key="String(sku.id)" class="sku" :class="{ active: Number(selectedSku.id) === Number(sku.id) }" @click="selectSku(sku)">
            <view><text>{{ sku.skuName }}</text><label>{{ sku.serviceMinutes ? `约 ${sku.serviceMinutes} 分钟 · ` : '' }}{{ sku.playerCount || 1 }} 位陪玩</label></view><text class="sku-price">¥{{ money(sku.price) }}{{ sku.priceType === 'PER_PLAYER' ? '/人' : '' }}</text>
          </view>
        </view>
      </view>
