...
> 更新日期：2026-08-16  
> 适用范围：`peiwan-platform` 当前代码与 V52 MySQL 数据库结构基线
...
商品分类是面向顾客的货架导航，回答“想玩哪个模式或场景”，例如三角洲行动的
“烽火地带”、无畏契约的“竞技排位”。基础服务则回答“陪玩师具体交付什么”，
并承担按陪玩等级、计价单位配置价格的职责。一个商品分类可以包含多种基础服务，
例如“竞技排位”分类下可以分别创建排位陪练、保段护航和复盘教学商品；不要把
`COMPANION`、`RANKING`、`TEACHING` 等基础服务类型原样复制成一套商品分类。
...
- 限量库存充足；
- 指定陪玩师时，其当前游戏资料已经审核；
- 已填写的陪玩等级、游戏ID/UID、区服、当前段位和目标段位与商品所属游戏匹配；
- 联系人和 11 位手机号完整；
- 游戏资料为选填；存在已保存或最近使用的资料时自动回填，也可清空后下单。
...
4. `pw_order_game_profile`：本单可选游戏ID/UID、昵称、区服和段位快照；
...
订单最终正常完成结算时，每次成功转出的成员按“订单基础单价 × 转单责任比例”承担责任，余额不足时允许形成负余额。转单链首个转出成员的责任金结算给该链最终完成服务的成员；链路中的中间转出成员责任金计入平台流水。
...
10. **售后与责任规则配置化**：转单责任比例、炸单责任比例已在“基础配置 → 服务责任规则”中配置，默认分别为 16% 和 20%。
