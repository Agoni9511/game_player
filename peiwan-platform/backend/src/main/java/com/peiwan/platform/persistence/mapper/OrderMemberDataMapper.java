...
public interface OrderMemberDataMapper {
 @Select("select id,order_status,required_player_count,assigned_player_id from pw_order where id=#{orderId} for update") Map<String,Object> lockOrder(long orderId);
 ...
 @Update("update pw_order set assigned_player_id=coalesce(assigned_player_id,#{playerId}),updated_at=current_timestamp where id=#{orderId}") int keepLegacyFirstMember(long orderId,long playerId);
 @Update("update pw_order set order_status='ASSIGNED',assigned_at=current_timestamp,updated_at=current_timestamp where id=#{orderId} and order_status='WAIT_ASSIGN'") int fillOrder(long orderId);
 @Select("select m.id,m.order_id,m.player_id,m.member_status,m.join_source,m.dispatch_task_id,m.joined_at,m.service_started_at,m.completed_at,m.cancelled_at,p.player_no,p.nickname player_name,p.avatar_url from pw_order_member m join pw_player p on p.id=m.player_id where m.order_id=#{orderId} order by m.id") List<Map<String,Object>> members(long orderId);
 ...
 @Select("select count(*) from pw_order_member m join pw_order o on o.id=m.order_id where m.player_id=#{playerId} and m.member_status in('ACCEPTED','IN_SERVICE') and o.order_status in('WAIT_ASSIGN','ASSIGNED','IN_SERVICE','PENDING_CONFIRM','WAIT_CUSTOMER_CONFIRM','AFTER_SALE')") long activeOrderCount(long playerId);
}
