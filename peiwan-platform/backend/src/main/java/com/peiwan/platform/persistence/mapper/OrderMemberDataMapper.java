...
public interface OrderMemberDataMapper {
 ...
 @Select("select count(*) from pw_order_member where order_id=#{orderId} and player_id=#{playerId}") int memberExistsAnyStatus(long orderId,long playerId);
 @Select("select m.id,m.order_id,m.player_id,m.member_status,m.join_source,m.dispatch_task_id,m.joined_at,m.service_started_at,m.completed_at,m.cancelled_at,p.player_no,p.nickname player_name,p.avatar_url from pw_order_member m join pw_player p on p.id=m.player_id where m.order_id=#{orderId} and m.player_id=#{playerId}") Map<String,Object> member(long orderId,long playerId);
