...
public interface WalletDataMapper{
 ...
 @Select("select m.*,l.level_code,l.level_name,l.level_no,l.min_recharge_amount,l.benefit_description from pw_user_member m join pw_member_level l on l.id=m.level_id where m.user_id=#{uid} and m.enabled=true and l.enabled=true") Map<String,Object> member(long uid);
 @Select("select * from pw_member_level where enabled=true and min_recharge_amount<=#{amount} order by min_recharge_amount desc,level_no desc limit 1") Map<String,Object> matchedLevel(java.math.BigDecimal amount);
